...
**Why separate?** FETCH uses a sub-agent to keep main conversation context clean and enable more reliable retrieval with a fresh context window. CREATE must use main agent since it needs full conversation context to extract reasoning.
...
4. **Two operations** - CREATE and FETCH have opposite context needs, architected separately
