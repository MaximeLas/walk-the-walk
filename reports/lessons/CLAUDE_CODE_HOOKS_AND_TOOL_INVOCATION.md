...
**Note:** This document was generated by Claude during a session where these concepts were discovered in real-time. The user configuration served as a practical example of hook usage in production. The document was then updated when we discovered the tool descriptions were mysteriously absent from the CLI output.
